...
export default async function CreatorEvent({ slug }: { slug: string }) {
	...
	const data = await getEventWithTenAttendeesByCleanName(
		formattedProps ?? ""
	);
	...
	switch (true) {
		case isBefore(today, eventStartDate) &&
			...
			isToday = false;
			break;
		case isAfter(today, eventEndDate):
			message = "Event is over";
			isToday = false;
			break;
		case extraInfo.registrationClosed:
	...
	return (
		<>
			...
			<div
				className={`
        ${
			isBefore(today, eventEndDate) || isSameDay(today, eventEndDate)
				? " py-[20px]"
				: " pt-[70px]"
		} px-[10px]`}
			>
				<div className="">
					...
					<p className=" text-[16px]">
						{message},{" "}
						{format(data?.event?.startDate ?? new Date(), "PPPP")}
					</p>
				...
				<div className="">
					...
					<div className=" mt-[20px]  flex flex-col gap-5">
						...
						<div className=" py-[15px] rounded-[12px] px-[15px] flex gap-3 items-center bg-secondary ">
							...
							<div className="">
								<p>
									{formatWithCommas(tickets?.available ?? 0)}
								</p>
					...
					<>
						{data?.attendees.length !== 0 ? (
							...
							<p className=" mt-[10px] pb-[20px] text-accent">
								No attendees yet, check this space later for an
								update.
							</p>
						)}
					</>
				...
				<CloseRegistration event={extraInfo} />

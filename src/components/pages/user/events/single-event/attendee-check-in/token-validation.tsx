  ...
  useEffect(() => {
    ...
    const fetchAttendee = async () => {
      try {
        ...
        const ticketInfo = await getTicketByID(info?.attendee.ticketId ?? "");
        if (!info?.attendee) {
          setInfo(null);
          setIsInvalid(true);
          return;
        ...
        <div className="">
              ...
              {info?.attendee?.eventId !== event.id ? (
                <>
                  <div className="flex items-center mt-[10px]">
                    <p>
                      This code does not belong to this event, try using a
                      different code or contact support
                    </p>
                  ...
                  <div className=" ">
                    ...
                    {info.attendee?.checkedIn ? (
                      <>
                        <p className="">
                          Entry time:{" "}
                          {format(info.attendee?.checkedIn, "eeee, LLLL c ")}
                        </p>
                        <p className=" text-[14px] text-accent">
                          {format(info.attendee?.checkedIn, "p")} · GMT +1 ·{" "}
                          {info.ticket.isFree ? "Free Ticket" : "Paid Ticket"}
                        </p>
                      </>
                    ) : (
                      <>
                        {info.event?.startDate && (
                          <>
                            <p>
                              Event time:{" "}
                              {format(info.event?.startDate, "eeee, LLLL c ")}
                            </p>
                            <p className=" text-[14px] text-accent">
                              {info?.event?.time} · GMT +1 ·{" "}
                              {info.ticket.isFree
                                ? "Free Ticket"
                                : "Paid Ticket"}
                            </p>
            ...
            <>
              {isInvalid &&
                ...
                info?.attendee === null && (
                  <div className=" flex items-center mt-[10px]">
                    <p>
                      Invalid access code, try rechecking the code or contact
                      support
                    </p>
                    <Button className=" items-center flex">
                      <FaPhoneAlt />
          ...
          {(isInvalid === false || inputValue.length !== 6) && (
            <Button
              type="submit"
              disabled={
                info === null || loading || info?.attendee?.eventId !== event.id
              }
            ...
            >
              ...
              {loading && info !== null ? "Checking in" : "Check in"}
            </Button>
          )}
        </div>
      </div>

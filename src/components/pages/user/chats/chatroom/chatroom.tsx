			...
			if (isTyping) {
				setTimeout(() => {
					setTypingUsers((prev) => {
						...
						if (
							existing &&
							Date.now() - existing.timestamp > 4500
						) {
							newMap.delete(userId);
		...
		async (
			...
			const optimisticMessage: SingleChatMessageProps = {
				...
				sender: {
					...
					displayPicture:
						attendee?.displayPicture ||
						session.auth.user.displayPicture,
				},
				message: messageText,
				type,
		...
		<>
			{!isConnected ? (
				...
				<div className=" relative h-[calc(100svh-55px)] py-[5px] px-[10px]">
					...
					>
						{chatRoomType === "group" && (
							<div className=" flex flex-col py-[20px] rounded-[30px] items-center w-full my-[10px] px-[20px] bg-secondary">
								<Image
									src={
								...
								/>
								<p className=" mt-[10px] capitalize">
									{event.title.toLowerCase()}
								</p>
								<p className=" text-center text-[13px] text-accent">
									This chatroom is only active as long as the
					...
					<ChatInput
						...
						placeholder={
							isConnected ? "Write message..." : "Connecting..."
						}
					/>
				</div>
			)}

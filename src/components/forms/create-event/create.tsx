...
import CalendarSelect from "./form-controller/calendar-select";
import googleCalendarUrl from "@/components/pages/user/events/calendar/google-calendar";
import { downloadICS } from "@/components/pages/user/events/calendar/apple-calendar";
	...
	const handleSubmit = async (data: eventCreationFormValues) => {
		...
		try {
			...
			if (response.ok) {
				...
				form.reset();
				if (calendarSelection === "google") {
					window.open(googleCalendarUrl({ event }), "_blank");
				}

				if (calendarSelection === "apple") {
					downloadICS({ event });
				}

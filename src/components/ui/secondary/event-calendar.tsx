...
export default function EventCalendar({
	...
	useEffect(() => {
		const displayEvent = async () => {
			...
			const going = await getFutureTicketsForCalendarByUserId(
				userId ?? ""
			);
			const hosting = await getUserEVentsForCalendar({});
			const interested = await getWatchlistForCalendarUserID(
				userId ?? ""
			);
			...
			>
				...
				>
					<p className="text-[14px]">
						{shortenToThree(
							calendarMonths.find((m) => m.number === month)
								?.month ?? ""
						)}{" "}
						{year}{" "}
					</p>
